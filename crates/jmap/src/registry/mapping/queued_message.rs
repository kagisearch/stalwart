...
use smtp::queue::{
    ...
    FROM_UNAUTHENTICATED, FROM_UNAUTHENTICATED_DMARC, Message, MessageWrapper, RCPT_DSN_SENT,
    RCPT_SPAM_PAYLOAD, Schedule, Status, spool::SmtpSpool,
};
use std::str::FromStr;
use store::{
...
fn map_message(message_in: &ArchivedMessage) -> QueuedMessage {
    ...
    for (bit, flag) in [
        ...
        (FROM_AUTOGENERATED | FROM_SIEVE, MessageFlag::Autogenerated),
    ] {
        if flags & bit != 0 {
