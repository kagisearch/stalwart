...
use crate::{
    ...
    message::delivery::{AutogeneratedMessage, IngestRecipient},
};
...
pub struct SieveIngestResult {
    pub messages: Vec<SieveOutputMessage>,
...
pub trait SieveScriptIngest: Sync + Send {
    ...
    ) -> impl Future<Output = trc::Result<SieveIngestResult>> + Send;
...
impl SieveScriptIngest for Server {
    ...
    async fn sieve_script_ingest(
        ...
        active_script: ActiveScript,
        autogenerated: &mut Vec<AutogeneratedMessage>,
    ) -> trc::Result<SieveIngestResult> {
        // Fetch raw message from blob storage
        let raw_message = self
            ...
            .ok_or_else(|| {
                trc::StoreEvent::NotFound
                    .into_err()
                    .caused_by(trc::location!())
                    .ctx(trc::Key::Reason, "Message blob not found")
            })?;
        ...
        let mut messages: Vec<SieveMessage> = vec![SieveMessage {
            raw_message: raw_message.as_slice().into(),
            file_into: Vec::new(),
            flags: Vec::new(),
            did_file_into: false,
        }];
            ...
            match event {
                Ok(event) => match event {
                    ...
                    Event::SendMessage {
                        ...
                        input = true.into();
                        if !access_token.has_permission(Permission::EmailSend) {
                            trc::event!(
                                Security(trc::SecurityEvent::Unauthorized),
                                AccountId = account_id,
                                SpanId = session_id,
                                Details = "emailSend",
                                Reason = "Blocked Sieve-generated outbound message",
                            );
                            continue;
                        }
        ...
        let primary_changed = instance.has_message_changed();
        let output: Vec<SieveOutputMessage> = messages
            .into_iter()
            .enumerate()
            .map(|(idx, msg)| SieveOutputMessage {
                raw: msg.raw_message.into_owned(),
                mailbox_ids: msg.file_into,
                keywords: msg.flags,
                did_file_into: msg.did_file_into,
                changed: if idx == 0 { primary_changed } else { true },
            })
            .filter(|msg| !msg.mailbox_ids.is_empty())
            .collect();

        Ok(SieveIngestResult {
            messages: output,
            reject_reason: reject_reason.clone(),
            discarded: do_discard || reject_reason.is_some(),
        })
    }
